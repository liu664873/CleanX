...
object NotificationHelper {
    ...
    fun showToolNotifications(context: Context) {
        ...
        toolNotifications().forEachIndexed { index, item ->
            val notification = NotificationCompat.Builder(context, NotificationChannelManager.TRIGGERED_TOOLS_CHANNEL_ID)
    ...
    private fun targetIntent(context: Context, route: String, requestCode: Int): PendingIntent {
    ...
    private data class ToolNotification(val title: String, val description: String, val route: String)

    private fun toolNotifications(): List<ToolNotification> = listOf(
    ...
    private fun defaultPackages(context: Context): Set<String> {
        return listOf("com.tencent.wemeet.app", "com.google.android.youtube", "com.spotify.music")
            .filter { it in context.packageManager.getInstalledApplications(0).map { a -> a.packageName }.toSet() }
            .toSet()
            .ifEmpty { emptySet() }
    }
