...
class NotificationRepositoryImpl(context: Context) : NotificationRepository {
    ...
    override fun hasNotificationListenerAccess(): Boolean =
        NotificationDataSource.hasNotificationListenerAccess(appContext)

    override fun isNotificationBlockingEnabled(): Boolean =
        NotificationDataSource.isEnabled(appContext)
    ...
    override fun blockedNotificationCount(): Int =
        NotificationDataSource.blockedCount(appContext)

    override fun blockedNotificationCountsByPackage(): Map<String, Int> =
        NotificationDataSource.blockedCountsByPackage(appContext)

    override fun selectedNotificationPackages(): Set<String> =
        NotificationDataSource.selectedPackages(appContext)

    override fun notificationApps(): List<BlockableNotificationApp> =
        NotificationDataSource.apps(appContext)
    ...
    override fun notificationListenerSettingsIntent(): Intent =
        NotificationDataSource.listenerSettingsIntent()
    ...
    override fun appDetailsSettingsIntent(packageName: String): Intent =
        NotificationDataSource.appDetailsSettingsIntent(packageName)
}
