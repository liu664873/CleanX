...
import com.quickcleanpro.phonecleaner.domain.model.toolbox.formatDuration
...
object AppUsageDataSource {
    ...
    fun openAppInfo(context: Context, packageName: String): Boolean =
        runCatching {
            context.startActivity(appInfoIntent(packageName))
            true
        }.getOrDefault(false)

    fun stopApp(context: Context, packageName: String): Boolean =
        if (packageName == context.packageName) false else openAppInfo(context, packageName)
    ...
    fun runningPackages(context: Context, packageNames: Set<String>): Set<String> {
        ...
        while (events.hasNextEvent()) {
            ...
            when (event.eventType) {
                ...
                UsageEvents.Event.ACTIVITY_RESUMED -> running.add(packageName)
                UsageEvents.Event.MOVE_TO_BACKGROUND,
                UsageEvents.Event.ACTIVITY_PAUSED,
                UsageEvents.Event.ACTIVITY_STOPPED -> running.remove(packageName)
            }
        }
        return running
    }
    ...
    suspend fun usageBetween(context: Context, startMillis: Long, endMillis: Long): List<AppUsageInfo> = withContext(Dispatchers.IO) {
        ...
        val events = runCatching { manager.queryEvents(queryStartMillis, endMillis) }.getOrNull()
